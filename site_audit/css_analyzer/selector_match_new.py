...
def specificity(selector: str) -> int:
    ...
    tags = len(re.findall(r'(?<![.#])([a-zA-Z][\w-]*)', selector.split(":")[0]))
    pseudo_keywords = {"hover", "focus", "active", "visited", "link",
                       "first", "last", "nth", "not", "where", "has", "is",
                       "before", "after", "root"}
    for kw in pseudo_keywords:
        if kw in selector.split("{")[0]:
            tags -= 1
    tags = max(0, tags)
