...
## Alternative: PicFlex (ChatGPT-Image2) for Production-Quality Illustrations

When the default backend produces insufficient quality, PicFlex via the `picflex-hermes` skill offers **ChatGPT-Image2** at 12 credits/generation — significantly better for editorial/poster-style illustrations. Compatible with both article cover and inline illustration use cases.

**Quick check:**
1. Credits: `curl` against `https://www.picflex.app/api/open/credits/balance` with Bearer token
2. Spendable ≥ 12 required for ChatGPT-Image2
3. Create task: `POST /api/open/text2image/tasks` with `model: gpt-image-2-text-to-image`
4. Poll: `GET /api/open/text2image/tasks/{taskId}` every ~5s until `completed`

See `free-image-generation` skill for complete curl patterns, Python alternatives, and SSL workaround notes.
