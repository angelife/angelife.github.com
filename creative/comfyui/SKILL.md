...
## Quick Fallback: Pollinations (No GPU / No API Key)

If you have no GPU, no ComfyUI, and no Comfy Cloud key, use **Pollinations.ai** for article covers and prototyping:

```bash
curl -s "https://image.pollinations.ai/prompt/$(python3 -c "import urllib.parse; print(urllib.parse.quote('your prompt here'))")" \
  -o /tmp/cover.png --max-time 120 -w "%{http_code}"
```

- **No setup, no GPU, no API key** — works from any internet-connected environment
- Output: 1024×1024 PNG
- Use for: article covers, social media thumbnails, quick prototypes
- Limitations: no seed control, no resolution options, no ControlNet
- For quality work (8K, inpainting, style control): use ComfyUI with local GPU or Comfy Cloud

See `free-image-generation` skill for full prompt engineering guide.
