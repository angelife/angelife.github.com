...
Vision is configured via `auxiliary.vision` in `config.yaml`:

```yaml
auxiliary:
  vision:
    provider: google       # or nvidia, openai, etc.
    model: gemini-2.0-flash # model name
    api_key: ''             # leave empty — key lives in .env
    base_url: ''
    timeout: 120
    extra_body: {}
    download_timeout: 30
...
**API key placement:** Keys go in `~/.hermes/.env`, NOT in `config.yaml`. `config.yaml` is protected; attempts to write it directly will be denied. The `api_key` field in `config.yaml` should remain empty — Hermes reads from `.env` automatically.

**Supported providers and their key env vars:**

| Provider | Env var |
|----------|---------|
| Google Gemini | `GOOGLE_API_KEY` or `GEMINI_API_KEY` |
| OpenAI / Azure | `OPENAI_API_KEY` |
| NVIDIA (via Integrate API) | `NVIDIA_API_KEY` |
...
**Typical errors:**
- `429 RESOURCE_EXHAUSTED` — free tier quota exceeded (Gemini free tier: ~15 req/min, 1M tokens/month)
- `401 Unauthorized` — wrong or expired API key
- `403 Forbidden` — billing not enabled on Google AI Studio

**In Docker containers:** The container `HERMES_HOME` (`/opt/data`) is pre-mounted from the host. Both `config.yaml` and `.env` are shared. If `hermes` CLI is not available inside the container (common in minimal images), configuration is done on the host Mac, not inside the container.

**Free tier limits (Google Gemini):** gemini-2.0-flash: 15 requests/minute, 1M tokens/minute on the free tier. For production vision use, enable billing on [ai.google.dev](https://ai.google.dev) to remove limits.
...
### Telegram: Bot Not Responding to @Mentions in Groups

See `references/telegram-privacy-mode.md` for the full walkdown.

**TL;DR**: Add `@HermesAgentBot` as a group administrator (no special permissions needed). This disables Telegram's Privacy Mode so the bot receives messages that `@mention` it.

### Gateway issues

#### Log locations (two distinct streams)

There are **two separate log files**, not one:

| Log | Path | Content |
|-----|------|---------|
| **Operational log** | `$HERMES_HOME/logs/gateway.log` (e.g. `/opt/data/logs/gateway.log`) | Structured platform logs: Telegram connect/disconnect, inbound messages, response timings, memory monitor, cron ticks, agent budget. **This is the primary debugging log.** |
| **s6-log pipe** | `$HERMES_HOME/logs/gateways/default/current` | Startup banner + anything that hit stdout before s6 pipeline was fully established. Often truncated, stale, or empty. **Secondary — check this only when operational log is silent.** |

Start with the operational log:
```bash
grep -E "ERROR|WARNING|failed|crash|429|timeout" $HERMES_HOME/logs/gateway.log | tail -20
```

#### Gateway process appears to be running but Telegram is down

The s6-supervise scaffold (process tree) outlives the actual gateway worker. Three things to check:

**1. Is the actual `hermes gateway run` process alive?**
```bash
ps aux | grep "hermes gateway run" | grep -v grep
```
If this returns nothing, the gateway worker has died even though s6-supervise shows on the process list.

**2. Is there a `down` file?**
```bash
ls -la /run/service/gateway-default/down
```
If `down` exists, s6-supervise is deliberately NOT respawning the worker. Fix:
```bash
rm /run/service/gateway-default/down
/command/s6-svc -u /run/service/gateway-default/
```
Wait 5-10 seconds, then check if `hermes gateway run` appears in `ps aux`.

**3. Is the lock file stale?**
Check `gateway.lock` — often at `$HERMES_HOME/gateway.lock` or via a virtiofs-mounted path (e.g. `/run/host_virtiofs/Users/macos/.hermes-docker/minimaxlab/gateway.lock` in Docker). A stale lock prevents a new gateway process from starting.

#### Gateway crash from provider rate limits

When a provider (e.g. NVIDIA) returns `HTTP 429 Too Many Requests` on all 3 retries, the gateway worker exits permanently. The operational log will show:
```
⏱️ Rate limited. Waiting Xs (attempt N/3)...
⚠️ Max retries (3) exhausted — trying fallback...
💀 Final error: HTTP 429: Error code: 429
```

**Fix**: Restart the gateway (see above). If rate limits persist, the provider limit is the bottleneck — reduce `api_max_retries` won't help, but increasing retry backoff or switching to a less-contended model might.

#### Other common gateway problems
